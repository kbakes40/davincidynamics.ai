...
const WEB_STATUS_OPTS: ("" | WebsiteStatus)[] = ["", "no_website", "weak_website", "has_website", "unknown"];
const PRIORITY_OPTS: ("" | LeadEnginePriorityLevel)[] = ["", "low", "medium", "high", "urgent"];
const STATUS_OPTS: ("" | LeadWorkflowStatus)[] = [
  "",
  "new",
  "researched",
  "drafted",
...
export function LeadFilters({
  ...
  onStatusFilter: (v: LeadWorkflowStatus | "") => void;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading">Stage</span>
        <select
          value={stageFilter}
        ...
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading ml-2">Status</span>
        <select
          value={statusFilter}
          onChange={e => onStatusFilter((e.target.value || "") as LeadWorkflowStatus | "")}
          className="h-8 rounded-lg border border-white/12 bg-background/50 px-2 text-xs font-heading text-foreground"
        >
          {STATUS_OPTS.map(s => (
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading">Source</span>
        <input
          value={sourceFilter}
        ...
        />
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading ml-1">Site</span>
        <select
          value={websiteStatusFilter}
          onChange={e => onWebsiteStatusFilter(e.target.value)}
          className="h-8 rounded-lg border border-white/12 bg-background/50 px-2 text-xs font-heading text-foreground max-w-[10rem]"
        >
        ...
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading ml-1">Pri</span>
        <select
        ...
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading ml-1">Cat</span>
        <input
        ...
        />
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading ml-1">City</span>
        <input
          value={cityFilter}
          ...
          className="h-8 w-28 rounded-lg border border-white/12 bg-background/50 px-2 text-xs font-heading text-foreground"
        />
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground font-heading ml-1">St</span>
        <input
          value={stateFilter}
